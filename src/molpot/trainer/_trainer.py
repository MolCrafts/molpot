...
class Trainer(BaseTrainer):
    def __init__(
        ...
        self.log_fields = LogAdapter.get_fields()
    ...
    def train(self, nsteps: int):
        ...
        outputs["elaspsed_time"] = self.config["report_rate"]
        self.model.train()
        ...
        while True:
            ...
            for inputs in self.train_data_loader:
                ...
                loss = self.criterion(train_output)
                outputs["train"]["_loss"] = loss
                for field in self.log_fields:
                    outputs["train"][field] = train_output[field]
                loss.backward()
                ...
                if nstep % self.config["valid_rate"] == 0:
                    ...
                    for inputs in self.valid_data_loader:
                        valid_output = self.model(inputs)
                        loss = self.criterion(valid_output)
                        outputs["valid"]["_loss"] = loss
                        for field in self.log_fields:
                            outputs["valid"][field] = valid_output[field]
