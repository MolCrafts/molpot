...
from molpot.potential import nnp
...
class GatedEquivariantBlock(nn.Module):
    ...
    def __init__(
        ...
        self.mix_vectors = nnp.Dense(n_vin, 2 * n_vout, activation=None, bias=False)
        self.scalar_net = nn.Sequential(
            nnp.Dense(n_sin + n_vout, n_hidden, activation=activation),
            nnp.Dense(n_hidden, n_sout + n_vout, activation=None),
        )
