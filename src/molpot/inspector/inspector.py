...
class Inspector:
    def __init__(
        ...
        **kwargs: Any,
        ...
        if row_settings is None:
            self.rows = DEFAULT_ROW_SETTINGS
        else:
            self.rows = {RowSettings(name) for name in row_settings}

        if mode is None:
            self.model_mode = Mode.EVAL
        else:
            self.model_mode = Mode(mode)

        if self.verbose is None:
            # pylint: disable=no-member
            self.verbose = 0 if hasattr(sys, "ps1") and sys.ps1 else 1
        ...
        validate_user_params(
            input_data, input_size, columns, col_width, device, dtypes, self.verbose
        )
        ...
        self.dtypes = dtypes
        self.model = model
        self._cached_forward_pass: dict[str, list[LayerInfo]] = {}
    ...
    def inspect(self, x):
        ...
        if model_name in self._cached_forward_pass:
            return self._cached_forward_pass[model_name]

        x, correct_input_size = process_input(
            self.input_data, self.input_size, self.batch_dim, self.device, self.dtypes
        )
        summary_list = forward_pass(
            self.model,
            x,
            self.batch_dim,
            self._cached_forward_pass,
            self.device,
            self.model_mode,
            **self.kwargs,
        )
        formatting = FormattingOptions(
            self.depth, self.verbose, self.columns, self.col_width, self.rows
        )
        ...
        self._cached_forward_pass[model_name] = summary_list

        return results

    def __call__(self, x):
        return self.inspect(x)
