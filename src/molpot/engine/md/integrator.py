...
from .barostat import BaseBarostat

#TODO: unit conversion module
kB = 1.38064852e-23
hbar = 1.0545718e-34
...
class RingPolymer(Integrator):
    ...
    def __init__(self, time_step: float, n_beads: int, temperature: float):
        ...
        self.omega = kB * n_beads * temperature / hbar
    ...
    def _init_propagator(self):
        ...
        Returns:
            ...
            torch.Tensor: Propagator with the dimension n_beads x 2 x 2,
                          where the last two dimensions mix the frames
        ...
        omega_normal = (
            2.0
            * self.omega
            * torch.sin(torch.arange(self.n_beads).float() * torch.pi / self.n_beads)
        )
    ...
    def _main_step(self, frame: Frame):
        ...
        positions_normal = frame["atoms", "positions_normal"]
        momenta_normal = frame["atoms", "momenta_normal"]

        # Propagate ring polymer
        frame["atoms", "momenta_normal"] = (
            self.propagator[:, 0, 0] * momenta_normal
            + self.propagator[:, 0, 1] * positions_normal * frame.masses
        )
        frame["atoms", "positions_normal"] = (
            self.propagator[:, 1, 0] * momenta_normal / frame.masses
...
class NPTVelocityVerlet(VelocityVerlet):
    ...
    Args:
        time_step (float): Integration time step in femto seconds.
        barostat (schnetpack.md.simulation_hooks.BaseBarostat): Barostat used for constant pressure dynamics.
    """

    ring_polymer = False
    pressure_control = True

    def __init__(self, time_step: float, barostat: BaseBarostat):
        super(NPTVelocityVerlet, self).__init__(time_step)
        self.barostat = barostat

    def half_step(self, frame: Frame):
        """
        Half steps propagating the frame and barostat momenta.

        Args:
            frame (object): Frame class containing all molecules and their
                             replicas.
        """
        self.barostat.propagate_half_step(frame)

    def _main_step(self, frame: Frame):
        """
        Main integrator step, where the barostat routine is used to propagate the frame positions and cells.
        """
        self.barostat.propagate_main_step(frame)
...
class NPTRingPolymer(RingPolymer):
    ...
    def __init__(
        self, time_step: float, n_beads: int, temperature: float, barostat: BaseBarostat
    ):
