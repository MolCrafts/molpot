...
def load_qm9()->tuple[mpot.DataLoader, mpot.DataLoader]:
    ...
    train_dataloader = mpot.create_dataloader(train.batch(batch_size=64).collate_frames(), nworkers=0)
    valid_dataloader = mpot.create_dataloader(valid.batch(batch_size=16).collate_frames(), nworkers=0)
